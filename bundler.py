...
(rootdir/"data"/"config"/"export"/"main"/"asset").mkdir(exist_ok=True,parents=True)
if (rootdir/"data"/"config"/"export"/"main"/"asset"/"assets.xml").exists():
    print("Assets are already there.")
    j=True
    with (rootdir/"data"/"config"/"export"/"main"/"asset"/"assets.xml").open() as fd:
    ...
    (rootdir/"data"/"config"/"export"/"main"/"asset"/"assets.xml").unlink(missing_ok=True)
    print("")

(rootdir/"data"/"config"/"gui").mkdir(exist_ok=True, parents=True)

if m&1<<1==1<<1:
    for root, dirs, files in (rootdir/"data"/"config"/"gui").walk(top_down=False):
        for name in files:
    ...
    class c(object):
        ...
        def open(s, f):
            if not isinstance(f,Path):
                if f.find("/")+f.find("\\")<0:f=(rootdir/"data"/"config"/"gui"/f)
...
with (rootdir/"data"/"config"/"export"/"main"/"asset"/"assets.xml").open("a") as g, langfiles() as tl:
